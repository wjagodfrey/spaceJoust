...
(function() {
  var Item, Item_Spawner, Player, addPlayerVelocity, applyPhysics, bombId, colors, entity, events, fireEvent, frameCount, frameTimeouts, gameCanvas, gameCq, gameKeys, gameMusic, gravity, hasBoxHit, hasMouseHit, item, level, levels, lightenDarkenColor, loadLevel, loaded, loadedFrameCount, mouse, mouseDownHandler, mouseMoveHandler, mouseUpHandler, onEvent, playOneOf, players, removePlayerVelocity, resizeFactor, root, setFrameTimeout, sound, soundsLoadedCount, soundsToLoad, touchDown, touchMove, touchTimeout, updateFrameTimeouts,
    __slice = [].slice,
    ...
    function PlayerSpawn(_at_x, _at_y, playerType) {
      ...
      this.controlsDisplay = {
        ...
        faderTimeout: setTimeout((function(_this) {
          return function() {
            console.log(_this.controlsDisplay);
    ...
    onrender: function(delta, time) {
      ...
      if (!loaded) {
        ...
        if (!level.winner) {
          ...
          level.render.save().font('2em Helvetica').textAlign('center').textBaseline('middle').fillStyle(players[level.winner.toLowerCase()].color).wrappedText(level.winner + " won!", level.width / 2, level.height / 2 - 10, level.width).restore();
        }
