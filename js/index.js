...
(function() {
  var Item, Item_Spawner, Player, addPlayerVelocity, applyPhysics, bombBackgroundColor, bombBehaviourIndicatorColor, bombId, entity, events, fireEvent, frameTimeouts, gameCanvas, gameCq, gravity, hasBoxHit, hasMouseHit, item, level, levels, loadLevel, loaded, mouse, mouseDownHandler, mouseMoveHandler, mouseUpHandler, onEvent, players, removePlayerVelocity, resizeFactor, root, setFrameTimeout, touchDown, touchMove, touchTimeout, updateFrameTimeouts,
    __slice = [].slice,
  ...
  entity.PlayerSpawn = (function() {
    ...
    PlayerSpawn.prototype.reset = function() {
      ...
      if ((_ref2 = this.player) != null) {
        _ref2.y = this.y + this.height / 2 - ((_ref3 = this.player) != null ? _ref3.height : void 0) / 2;
      }
      this.player.invincible = true;
      return this.player.invincibleTimeout = setFrameTimeout((function(_this) {
        return function() {
    ...
    PlayerSpawn.prototype.draw = function(ctx) {
      ...
      ctx.save().globalAlpha(0.2).fillStyle(this.player.color).fillRect(Math.round(this.x), Math.round(this.y), this.width, this.height).restore();
  ...
  entity.Slime = (function() {
    ...
    Slime.prototype.startGrowth = function() {
      return this.grow = true;
    };

    Slime.prototype.stopGrowth = function() {
    ...
    Slime.prototype.draw = function(ctx) {
      ...
      ctx.save().fillStyle('#7edc34').fillRect(Math.round(this.x), Math.round(this.y), this.width, this.height);
      _ref = this.bloops;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        bloop = _ref[_i];
  ...
  Player = (function() {
    ...
    Player.prototype.onHit = function(c, e, solid) {
      ...
      if (solid) {
        ...
        if (c.bottom) {
          ...
          if (!((_ref1 = this.keys.w) != null ? _ref1.pressed : void 0)) {
    ...
    Player.prototype.die = function(before, after) {
      if (!this.invincible) {
        ...
        if (level != null) {
          level.addBlinkUpdate(this.x, this.y, '-1', true);
        }
        this.lives--;
        this.vel = {
          x: 0,
    ...
    Player.prototype.draw = function(ctx, delta, time) {
      if (level != null) {
        ...
        return ctx.fillStyle('red').fillRect(Math.round((this.x + this.width / 2) - 1 + (this.direction > 0 ? +2 : -2)), Math.round(this.y + this.height / 2 - 1), 2, 2).restore();
      }
    };
    ...
    onrender: function(delta, time) {
      ...
      if (level != null) {
        ...
        if (!level.winner) {
          ...
          level.render.save().font('2em Helvetica').textAlign('center').textBaseline('middle').fillStyle(players[level.winner.toLowerCase()].color).wrappedText("The " + level.winner + " won!", level.width / 2, level.height / 2 - 10, level.width).restore();
        }
