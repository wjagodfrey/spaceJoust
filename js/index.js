...
(function() {
  var HUD, Item, Item_Spawner, Player, addPlayerVelocity, applyPhysics, bombId, entity, events, fireEvent, frame, frameCount, frameTimeouts, gameCanvas, gameCq, gravity, hasBoxHit, hasMouseHit, item, level, levels, loadLevel, loaded, mouse, mouseDownHandler, mouseMoveHandler, mouseUpHandler, onEvent, players, removePlayerVelocity, resizeFactor, root, setFrameTimeout, touchDown, touchMove, touchTimeout, updateFrameTimeouts,
    __slice = [].slice,
  ...
  updateFrameTimeouts = function() {
    ...
    frame = frameCount++;
    if (frameTimeouts[frame]) {
      _ref = frameTimeouts[frame];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        callback = _ref[_i];
        callback();
      }
      frameTimeouts[frame] = void 0;
      return delete frameTimeouts[frame];
    }
  };
  ...
  entity.PlayerSpawn = (function() {
    ...
    PlayerSpawn.prototype.draw = function(ctx) {
      ...
      lifeMeterHeight = this.height * (this.player.lives / this.player.maxLives);
      return ctx.save().globalAlpha(0.2).fillStyle('white').fillRect(Math.round(this.x), Math.round(this.y + (this.height - lifeMeterHeight)), this.width, lifeMeterHeight).restore();
    };
  ...
  entity.Bomb = (function() {
    ...
    Bomb.prototype.draw = function(ctx) {
      var behaviourIndicatorColor;
      if (!this.explode.exploding) {
        ctx.save().fillStyle('#0b0f16').fillRect(Math.round(this.x), Math.round(this.y), this.width, this.height).restore();
        behaviourIndicatorColor = '#425c95';
        if (this.xBounce) {
          ctx.save().fillStyle(behaviourIndicatorColor).fillRect(Math.round(this.x), Math.round(this.y + 2), this.width, this.height - 4).restore();
        }
        if (this.yBounce) {
          ctx.save().fillStyle(behaviourIndicatorColor).fillRect(Math.round(this.x + 2), Math.round(this.y), this.width - 4, this.height).restore();
        }
        return ctx.save().fillStyle(this.armed ? '#b70011' : '#7eac04').fillRect(Math.round(this.x + 1), Math.round(this.y + 1), this.width - 2, this.height - 2).restore();
      } else {
  ...
  item.Bomb = (function(_super) {
    ...
    Bomb.prototype.onHit = function(col, ent) {
      ...
      if (ent.type === 'Player' && (ent.item == null)) {
        ent.item = {
          ...
          draw: (function(_this) {
            return function(ctx) {
              var behaviourIndicatorColor, height, width;
              width = 5;
              height = 5;
              ...
              behaviourIndicatorColor = '#425c95';
              if (_this.xBounce) {
                ctx.save().fillStyle(behaviourIndicatorColor).fillRect(Math.round(ent.x + (ent.width - width) / 2), Math.round(ent.y + (ent.height - height) / 2 + 2), width, height - 4).restore();
              }
              if (_this.yBounce) {
                return ctx.save().fillStyle(behaviourIndicatorColor).fillRect(Math.round(ent.x + (ent.width - width) / 2 + 2), Math.round(ent.y + (ent.height - height) / 2), width - 4, height).restore();
              }
            };
          })(this)
        };
    ...
    Bomb.prototype.draw = function(ctx) {
      ...
      if (this.yBounce) {
        return ctx.save().fillStyle(behaviourIndicatorColor).fillRect(Math.round(this.x + 2), Math.round(this.y), this.width - 4, this.height).restore();
      }
    };
