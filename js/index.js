...
(function() {
  var Boundary, Button, HUD, Item_Spawner, Laser, Player, PlayerSpawn, addPlayerVelocity, events, fireEvent, frame, gameCanvas, gameCq, gravity, hasBoxHit, hasMouseHit, items, level, levels, loadLevel, loaded, mouse, mouseDownHandler, mouseMoveHandler, mouseUpHandler, onEvent, playerHitsAndVelocity, players, removePlayerVelocity, resizeFactor, root, touchDown, touchMove, touchTimeout,
    __slice = [].slice,
  ...
  items.AddLife = (function() {
    function AddLife(container, key, spawner) {
      ...
      console.log(this.container, this.key);
    }
    ...
    AddLife.prototype.draw = function(ctx) {
      return ctx.save().globalAlpha(0.5).fillStyle('#cb71ff').fillRect(Math.round(this.x), Math.round(this.y), this.width, this.height).restore();
    };
  ...
  items.NoWingsEnemy = (function() {
    ...
    NoWingsEnemy.prototype.onHit = function(col, ent) {
      var cache, player, _ref, _ref1, _ref2;
      if (ent.type === 'Player') {
        player = ent.other;
        cache = {
        ...
        player.addEffect('NoWingsEnemy', function() {
          ...
          return player.yForce = cache.yForce;
        }, 4000);
        if ((_ref = this.spawner) != null) {
          _ref.itemCount--;
        }
    ...
    NoWingsEnemy.prototype.draw = function(ctx) {
      return ctx.save().globalAlpha(0.5).fillStyle('#8aff58').fillRect(Math.round(this.x), Math.round(this.y), this.width, this.height).restore();
    };
  ...
  levels.level_middle = {
    ...
    onBuild: function() {
      ...
      this.spawner = new Item_Spawner(this.midground, ['AddLife', 'NoWingsEnemy'], this.width, this.height, 5);
